...
%%%%%%%%%%%%%%%%%%%%%%%
% Positions
pos(1,1). pos(2,2). pos(3,3). pos(10,10).

% Blocks
block(b1). block(b2). block(b3).
block(b4). block(b5). block(b6).

% Agents
agent(a1). agent(a2).

% (Opzionale) risorse specifiche
...
% Move a block table→block
action(move_table_to_block_start(Agent, Block1, Block2, X1, Y1, X2, Y2),
  [ available(Agent), ontable(Block1),
    at(Block1, X1, Y1), at(Block2, X2, Y2),
    clear(Block2), clear(Block1)
  ],
  [ on(_, Block1), on(Block1, _),
    moving_table_to_table(_, Block1, _, _, _, _),
    moving_table_to_block(_, Block1, _, _, _, _, _)
  ],
  [],
  [ agent(Agent), pos(X1, Y1), pos(X2, Y2),
    block(Block1), block(Block2), Block1 \= Block2
  ],
  [
    del(available(Agent)), del(clear(Block1)),
    del(ontable(Block1)), del(at(Block1, X1, Y1)),
...
% Move a block on→table
action(move_onblock_to_table_start(Agent, Block1, Block2, X1, Y1, X2, Y2),
  [ available(Agent), on(Block1, Block2),
    at(Block1, X1, Y1), at(Block2, X1, Y1), clear(Block1)
  ],
  [ moving_onblock_to_table(_, Block1, _, _, _, _), on(_, Block1),
    ontable(Block1), at(_, X2, Y2)
  ],
  [],
  [ agent(Agent), pos(X1, Y1), pos(X2, Y2),
    block(Block1), block(Block2), Block1 \= Block2
...
action(move_onblock_to_table_end(Agent, Block1, X1, Y1, X2, Y2),
  ...
  [ at(_, X2, Y2) ],
  [],
  [ agent(Agent), block(Block1), pos(X1, Y1), pos(X2, Y2) ],
  [
...
% Move a block on→block (corretto)
action(move_onblock_to_block_start(Agent, Block1, Block2, Block3, X1, Y1, X2, Y2),
  [ available(Agent), 
    on(Block1, Block3), at(Block1, X1, Y1), at(Block3, X1, Y1),
    at(Block2, X2, Y2), clear(Block2), clear(Block1)
  ],
  [ moving_onblock_to_block(_, Block1, _, _, _, _, _, _), on(_, Block1), ontable(Block1) ],
  [],
  [ agent(Agent), pos(X1, Y1), pos(X2, Y2),
    block(Block1), block(Block2), block(Block3),
    Block1 \= Block2, Block1 \= Block3, Block2 \= Block3
  ],
  [
    del(available(Agent)), del(clear(Block1)), del(on(Block1, Block3)), del(at(Block1, X1, Y1)),
    add(moving_onblock_to_block(Agent, Block1, Block2, Block3, X1, Y1, X2, Y2)),
    add(clear(Block3))
  ]
).
action(move_onblock_to_block_end(Agent, Block1, Block2, Block3, X1, Y1, X2, Y2),
  [ moving_onblock_to_block(Agent, Block1, Block2, Block3, X1, Y1, X2, Y2), clear(Block2) ],
  [],
  [],
  [ agent(Agent), block(Block1), block(Block2), block(Block3), pos(X1, Y1), pos(X2, Y2) ],
  [
    del(clear(Block2)), del(moving_onblock_to_block(Agent, Block1, Block2, Block3, X1, Y1, X2, Y2)),
    add(on(Block1, Block2)), add(at(Block1, X2, Y2)), add(clear(Block1)), add(available(Agent))
  ]
).