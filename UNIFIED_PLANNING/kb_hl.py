...
# variables for universal quantification
any_block = Variable("any_block", Block)
any_pos = Variable("any_pos", Pos)
any_agent = Variable("any_agent", Agent)

move_table_to_table_start.add_precondition(
    Forall(
        Implies(Not(Equals(any_block, block)), Not(at(any_block, to_pos))),
        any_block
    )
)
...
move_table_to_table_start.add_precondition(
    Forall(
        Implies(
            Equals(any_block, block),
            Not(moving_table_to_table(any_agent, any_block, any_pos, any_pos))
        ),
        any_agent, any_block, any_pos, any_pos
    )
)
# Alternativamente, invece di `Forall`, un approccio come:
""" for a in agents:
    for b in blocks:
        for fp in positions:
            for tp in positions:
                if b == block: 
                    # This is the Not(moving_table_to_table(...))
                    move_table_to_table_start.add_precondition(
                        Not(moving_table_to_table(a, b, fp, tp))
                    ) """


any_block2 = Variable("any_block2", Block)
move_table_to_table_start.add_precondition(
    Forall(
        Implies(
            Equals(any_block, block),
        ...
        any_agent, any_block, any_block2, any_pos, any_pos
    )
)
...
move_table_to_block_start.add_precondition(
    Forall(
        Implies(Not(Equals(any_block, block1)), Not(on(any_block, block1))),
        any_block
    )
)
...
move_table_to_block_start.add_effect(moving_table_to_block(
                agent, block1, block2, from_pos, to_pos), True
                                    )
...
move_onblock_to_table_start.add_precondition(Not(Equals(block1, block2)))  # Blocks must be different
...
move_onblock_to_table_start.add_precondition(
    Forall(
        Implies(Not(Equals(any_block, block1)), Not(at(any_block, to_pos))),
        any_block
    )
)
...
move_onblock_to_table_start.add_precondition(
    Forall(
        Implies(
            Equals(any_block, block1),
            Not(moving_onblock_to_table(any_agent, any_block, any_pos, any_pos))
        ),
        any_agent, any_block, any_pos, any_pos
    )
)
